...
export default function sketch(p) {
  p.setup = function () {
    ...
    messAmount = 80; // Control the amount of distortion
  ...
  function drawEvenWire(i, j, weavingValue) {
    ...
    if (i === 0) {
      p.vertex(grid[j][i][0], grid[j][i][1]); // Add the first anchor point.
    } else {
  ...
  function drawOddWire(i, j, weavingValue) {
    ...
    if (i === 0) {
      p.vertex(grid[j + 1][i][0], grid[j + 1][i][1]); // Add the first anchor point.
    } else {
      const isEvenIndex = i % 2 === 0;
